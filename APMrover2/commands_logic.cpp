...
{
	if(wp_skipping() && g.wp_skipper_on)
   	{
		gcs().send_text(MAV_SEVERITY_INFO, "Waypoint #%u Skipped, with Manuel Control @%dm",
                static_cast<uint32_t>(cmd.index),
                static_cast<int32_t>(fabsf(wp_distance)));
        	return true;
   	}
	//gcs().send_text(MAV_SEVERITY_INFO, "Verifying Nav WP");
    ...
    if (obstacle.turning == true && wp_distance <= g.rangefinder_wp_rad)
    {
        gcs().send_text(MAV_SEVERITY_INFO, "Waypoint #%u Skipped, with Lidar Radius %dm",
                static_cast<uint32_t>(cmd.index),
                static_cast<int32_t>(fabsf(wp_distance)));
        return true;
    }
