...
private:
    ...
    RC_Channel *channel_skip_wp; //INDRO WP SKIPPING
    ...
    //Used for indro turn prediction
     bool turning_right;

    //Indro WP Skipping functions
    bool current_wp_skip, previous_wp_skip;
    int current_skipper_val;

private:
    // private member functions
	/*Defined by Indro ~ Sam Swerhone*/
    bool wp_skipping();
    void turn_prediction();
    float bearing_btwn_loc(const struct Location& current, const struct Location& next); //Declared in Turn_Predict
    bool turn_is_right(float crnt_bearing, float nxt_bearing); //Declared in Turn_Predict
    bool get_is_turn_right();
	//End, ArduPilot APMrover2 Defaults
    void ahrs_update();
